...
$stats = $conex->query("SELECT 
    ...
    (SELECT COUNT(*) FROM cotizaciones WHERE estado_cotizacion = 'Aprobado' AND activo = 1) as aprobadas,
    (SELECT COUNT(*) FROM cotizaciones WHERE estado_cotizacion = 'Pendiente' AND activo = 1) as pendientes
...
<body>
    <div class="main-container">
        ...
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            ...
            <li class="nav-item" role="presentation">
                <button class="nav-link active" id="recientes-tab" data-bs-toggle="tab" data-bs-target="#recientes" type="button" role="tab" aria-controls="recientes" aria-selected="true">
            ...
            <div class="tab-pane fade" id="eliminar" role="tabpanel" aria-labelledby="eliminar-tab">
                ...
                <?php else: ?>
                    <div class="alert alert-info">
                        <i class="fas fa-info-circle"></i> No hay cotizaciones en la papelera.
                    </div>
                <?php endif; ?>
            </div>
        </div>
    ...
    <div class="options-floating-card" id="optionsCard">
        <div class="options-card-header">
            <h3 class="options-card-title" id="optionsClientName">Opciones de Cotización</h3>
    ...
    <script>
        ...
        // Función para mostrar opciones de una cotización
        function showOptions(id, clientName) {
            currentCotizacionId = id;
            
            // Actualizar el nombre del cliente en la cabecera
            document.getElementById('optionsClientName').textContent = 'Opciones: ' + clientName;
        ...
        // Función para restaurar una cotización
        function restoreCotizacion(id) {
            if (confirm('¿Estás seguro de que deseas restaurar esta cotización?')) {
                window.location.href = `restaurar_cotizacion.php?id=${id}`;
            }
        }
        ...
        document.getElementById('searchInput').addEventListener('input', function() {
            ...
            const allCotizacionesList = document.getElementById('allCotizacionesList');
            
            if (query.length === 0) {
                resultsContainer.style.display = 'none';
                allCotizacionesList.style.display = 'block';
                return;
            } else {
                allCotizacionesList.style.display = 'none';
            }
            ...
            fetch(`?ajax=1&q=${encodeURIComponent(query)}`)
                ...
                .then(data => {
                    ...
                    data.forEach(cotizacion => {
                        ...
                        item.innerHTML = `
                            <div onclick="showOptions(${cotizacion.id_cotizacion}, '${cotizacion.nombre_cliente.replace(/'/g, "\\'")}')">
                                <div class="search-client-name">${cotizacion.nombre_cliente}</div>
                                <div class="search-client-info">${cotizacion.marca_vehiculo} ${cotizacion.modelo_vehiculo} - ${cotizacion.placa_vehiculo}</div>
                                <div class="search-client-date">${new Date(cotizacion.fecha_cotizacion).toLocaleDateString()}</div>
                            </div>
                            <div>
                                <span class="client-card-badge">$${parseFloat(cotizacion.total_cotizacion).toFixed(2)}</span>
                            </div>
                        `;
        ...
        // Cerrar tarjeta de opciones con tecla ESC
